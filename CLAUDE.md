...
### Follow-up requests (sending mechanism built, no UI trigger yet)

`Interaction` has a nullable `parentInteractionId` field (links a follow-up to the interaction it continues) and `Answer` has a nullable `responseId` field (each provider's response id — OpenAI `resp_...`, Claude `msg_...`, GCP `responseId`, all confirmed live), both still unset by any *production* code path — nothing yet creates a follow-up `Interaction` from the UI. The thread model is a chain of separate linked `Interaction`s (one per turn, `parentInteractionId` pointing at the previous turn), not a multi-turn container inside a single `Interaction` — this keeps the 1-file-per-interaction storage layout and the 4-parallel-`AnswerVm` architecture unchanged.
...
Still missing before this is a real feature: a UI action that actually creates a follow-up `Interaction` (setting `parentInteractionId`) and calls `requestFollowUpAnswer` — that's `gptui.ui.viewmodel`/`gptui.ui.view` work, deliberately deferred.
